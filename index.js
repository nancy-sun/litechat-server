...
io.on("connection", (socket) => {
    ...
    socket.on("sendSgn", (payload) => {
        ...
        socket.to(payload.room).emit("voiceJoined", { signal: payload.signal, caller: payload.caller });
    });

    socket.on("returnSgn", (payload) => {
        // io.to(payload.caller).emit("receiveSgn", { signal: payload.signal, id: socket.id });
        socket.to(payload.room).emit("receiveSgn", { signal: payload.signal, id: socket.id });
    });
})
